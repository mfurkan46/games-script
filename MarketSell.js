  ...
  },
{
  ...
  const price2=text.match(/([1-9][0-9]*((,| )[0-9]{3})*|0)(\.[0-9]+)*[ ](\₺)/gm)[0].replace("₺","")
  run2func(Number(price2.replaceAll(".","")),value,minprice)
    ...
    function run2func(price,value,minprice){
      ...
      if(quantity==0){
       ...
       setTimeout(()=>{notquantity.remove()},2000)
      }
if (quantity!=0 && newprice>minprice) {
        fetch(`https://rivalregions.com/storage/newsell/${value}/${quantity}/${newprice}`, {
    method: 'POST',
    headers: {
      'accept': 'text/html, */*; q=0.01',
      'accept-language': 'tr-TR,tr;q=0.9,en-US;q=0.8,en;q=0.7',
    ...
    console.log(json)
    if(json==Number(json)){
            let time=5*60,r=document.getElementById('timer'),tmp=time;
    ...
    for (var i = 0; i < products.length; i++) {
     ...
     }else if(location.href.includes("storage")!=true && document.getElementById("selectlist")){
